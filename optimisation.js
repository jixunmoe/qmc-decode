const fs = require('fs');
const privKey = [
	
];

const newKey = [];
const derive = [];

// 提取出来的规律
const data = [27,28,31,36,43,52,63,76,91,108,127,148,171,196,223,252,27,60,95,132,171,212,255,44,91,140,191,244,43,100,159,220,27,92,159,228,43,116,191,12,91,172,255,84,171,4,95,188,27,124,223,68,171,20,127,236,91,204,63,180,43,164,31,156,27,156,31,164,43,180,63,204,91,236,127,20,171,68,223,124,27,188,95,4,171,84,255,172,91,12,191,116,43,228,159,92,27,220,159,100,43,244,191,140,91,44,255,212,171,132,95,60,27,252,223,196,171,148,127,108,91,76,63,52,43,36,31,28];

for (let i = 0; i < (0x7FFF * 7); i++)
{
	let j = i % 0x7FFF;
	const c = (j * j + 27) & 0xFF;

	if (i < data.length) newKey.push(privKey[c]);
	derive.push(privKey[c]);

	if (data[i % data.length] !== c)
	{
		throw new Error('failed to validate at ' + i);
	}
}

fs.writeFileSync('test.bin', Buffer.from(derive));

const keys_str = newKey.map((c, i) => '0x' + ('0' + c.toString(16)).slice(-2));
for (let i = 0; i < keys_str.length; i += 8)
{
	const line = keys_str.slice(i, i + 8);
	console.info(line.join(', ') + ',');
}
