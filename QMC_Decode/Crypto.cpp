#include "Crypto.h"

unsigned char privKey[256] = {
	
};


Crypto::Crypto()
{
}


Crypto::~Crypto()
{
}

void Crypto::encrypt(std::vector<uint8_t>& data, unsigned int offset, size_t size)
{
	if (size > data.size())
	{
		size = data.size();
	}

	for (unsigned int i = 0; i < size; i++)
	{
		data.at(i) = encode(data.at(i), offset + i);
	}
}

uint8_t Crypto::encode(uint8_t c, unsigned int i)
{
	unsigned int offset = i % 0x7FFF;
	unsigned int index = (offset * offset + 0x13C1B) & 0xFF;
	return privKey[index];
}
