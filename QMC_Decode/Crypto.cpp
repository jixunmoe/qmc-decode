#include "Crypto.h"

#define OPTIMISE_CRYPTO (1)

#if OPTIMISE_CRYPTO
unsigned char privKey[128] = {
	,
};
#else
unsigned char privKey[256] = {
	
};
#endif


Crypto::Crypto()
{
}


Crypto::~Crypto()
{
}

void Crypto::transform(uint8_t* data, unsigned int offset, size_t size)
{
	for (unsigned int i = 0; i < size; i++)
	{
		data[i] ^= encode(data[i], offset + i);
	}
}

void Crypto::transform(std::vector<uint8_t>& data, unsigned int offset, size_t size)
{
	if (size > data.size())
	{
		size = data.size();
	}

	transform(&data.front(), offset, size);
}

uint8_t Crypto::encode(uint8_t c, unsigned int i)
{
#if OPTIMISE_CRYPTO
	if (i > 0x7FFF) {
		// 腾讯傻逼算法写歪了
		return privKey[(i % 0x7FFF) & 0x7F];
	}
	else {
		return privKey[i & 0x7F];
	}
#else
	unsigned int offset = i > 0x7FFF ? (i % 0x7FFF) : i;
	unsigned int index = (offset * offset + /* 80923 & 0xFF = */ 27) & 0xFF;
	return privKey[index];
#endif
}
